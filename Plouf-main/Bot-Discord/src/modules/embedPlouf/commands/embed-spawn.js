        ...
        return __awaiter(this, void 0, void 0, function* () {
            yield executeRoleBasedCommand(interaction, 9, false, (interaction) => __awaiter(this, void 0, void 0, function* () {
                try {
                    ...
                    const rolesChannel = interaction.client.channels.cache.get(rolesChannelId);
                    // Vérifications des salons
                    if (!rulesChannel || !rolesChannel) {
                        yield interaction.reply({ content: 'Un des salons spécifiés est introuvable.', ephemeral: true });
                        return;
                    }
                    ...
                    yield Promise.all([clearMessages(rulesChannel), clearMessages(rolesChannel)]);
                    // Envoi des embeds français et anglais dans le salon des règles
                    const messageFr = yield rulesChannel.send({
                        embeds: [
                            new EmbedBuilder()
                                .setTitle('<:icon_folder:1327707360302731346> Règlement du Serveur')
                                .setDescription("Bienvenue sur notre serveur ! Merci de lire attentivement ces règles pour garantir une expérience agréable à tous les membres.")
                                .setColor(0x1d3557)
                                .addFields({ name: '<:icon_pin:1327705676419432518> Respect & Bienveillance', value: "Traitez les autres membres avec respect. Aucun harcèlement, insulte ou discours de haine ne sera toléré." }, { name: '<:icon_screen:1327705674800431124> Contenu Approprié', value: "Ne partagez que du contenu respectant les règles Discord et les lois en vigueur." }, { name: '<:icon_earth:1327705673181692066> Langues', value: "Ce serveur est **bilingue**. Vous pouvez utiliser le **français** ou l'**anglais**." }, { name: '<:icon_raid:1327705671227019415> Pas de Spam', value: "Évitez le spam, les répétitions inutiles et les mentions abusives." }, { name: '<:icon_message:1327705669448630384> Utilisation des Canaux', value: "Veuillez respecter la description des canaux et utiliser le bon canal pour vos discussions." }, { name: '<:icon_temp:1327705667879964774> Sanctions', value: "Tout non-respect des règles peut entraîner des avertissements, des restrictions, voire une exclusion." })
                    ...
                    const messageEn = yield rulesChannel.send({
                        embeds: [
                            new EmbedBuilder()
                                .setTitle('<:icon_folder:1327707360302731346> Server Rules')
                                .setDescription("Welcome to our server! Please carefully read these rules to ensure a pleasant experience for all members.")
                                .setColor(0x742a2a)
                                .addFields({ name: '<:icon_pin:1327705676419432518> Respect & Kindness', value: "Treat other members with respect. Harassment, insults, or hate speech will not be tolerated." }, { name: '<:icon_screen:1327705674800431124> Appropriate Content', value: "Only share content that complies with Discord's rules and applicable laws." }, { name: '<:icon_earth:1327705673181692066> Languages', value: "This server is **bilingual**. You may use either **French** or **English**." }, { name: '<:icon_raid:1327705671227019415> No Spam', value: "Avoid spamming, unnecessary repetition, and excessive mentions." }, { name: '<:icon_message:1327705669448630384> Channel Usage', value: "Please follow the channel descriptions and use the correct channel for your discussions." }, { name: '<:icon_temp:1327705667879964774> Sanctions', value: "Breaking these rules may result in warnings, restrictions, or bans." })
                    ...
                    yield rulesChannel.send({
                        embeds: [
                            new EmbedBuilder()
                                .setTitle('<:icon_folder:1327707360302731346> Sommaire des Règlements')
                                .setDescription(`Cliquez sur une langue pour accéder au règlement correspondant :\n\n` +
                    ...
                    const notifEmbed = new EmbedBuilder()
                        ...
                        .setDescription("Stay informed by subscribing to notifications! Select your preferences by clicking on the emojis below:\n\n" +
                    ...
                    const laneEmbed = new EmbedBuilder()
                        ...
                        .setDescription("Select your main role by clicking on the corresponding emoji:\n\n" +
                    ...
                    const laneButtons = new ActionRowBuilder().addComponents(new ButtonBuilder()
                        .setCustomId("toplane")
                    ...
                    const notifButtons = new ActionRowBuilder().addComponents(new ButtonBuilder()
                        ...
                        .setStyle(ButtonStyle.Secondary));
                    yield rolesChannel.send({ embeds: [laneEmbed], components: [laneButtons] });
                    yield rolesChannel.send({ embeds: [notifEmbed], components: [notifButtons] });
                    // Réponse finale
                    yield interaction.reply('Les règlements et les rôles ont été générés avec succès.');
                }
                catch (error) {
                    console.error(error);
